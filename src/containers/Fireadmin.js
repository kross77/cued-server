...
import Loader from '../components/preloaders/PulseLoader'
...
class Fireadmin extends Component {
  ...
  constructor (props) {
    ...
    this.makeTableCardForElementsInArray = this.makeTableCardForElementsInArray.bind(
      this)
  ...
  findFirebasePath () {
    ...
    var firebasePath = (this.props.route.path.replace('/fireadmin/', '').
    ...
    chunks.map((item, index) => {
      ...
      if (theInsertSchemaObject != null && theInsertSchemaObject &&
        ...
        if (isLastObject && theInsertSchemaObject != null) {
          theInsertSchemaObject = theInsertSchemaObject[Object.keys(
            theInsertSchemaObject)[0]]
        } else {
    ...
    paginator.on('value', function () {
      ...
      if (isJustArray) {
        ...
        ref.once('value').then(function (snapshot) {
          console.log(
            'Received compleete array with length of ' + snapshot.val().length)
    ...
    if (objToInsert == null) {
      ...
      this.setState({
        notifications: [
          {
            type: 'danger',
            content: 'We couldn\'t find object to insert. Check your schema settings.',
          }],
      })
      ...
      this.setState({
        notifications: [
          {
            type: 'success',
            content: 'Element added. You can find it in the table bellow.',
          }],
      })
      ...
      if (Config.adminConfig.goDirectlyInTheInsertedNode) {
        this.props.router.push(this.props.route.path.replace(':sub', '') +
          (this.props.params && this.props.params.sub
            ? this.props.params.sub
            : '') + Config.adminConfig.urlSeparator + keyToCreate)
      }
    }
  }
  ...
  addItemToArray (name, howLongItIs) {
    ...
    if (this.state.currentInsertStructure) {
      if (this.state.isJustArray) {
        ...
        correctPathToInsertIn = this.state.firebasePath + '/' + name + '/' +
          (parseInt(howLongItIs))
      }
    }
    ...
    if (dataToInsert != null) {
      ...
      firebase.app.database().
        ref(correctPathToInsertIn).
        set(dataToInsert).
        then(() => {
          this.setState(
            {notifications: [{type: 'success', content: 'Item is inserted.'}]})
          this.refreshDataAndHideNotification()
        })
    } else {
      this.setState({
        notifications: [
          {
            type: 'danger',
            content: 'Error 201: We are missing correct insert schema.',
          }],
      })
    }
  }
  ...
  deleteFieldAction (key, isItArrayItem = false) {
    ...
    var firebasePathToDelete = (this.props.route.path.replace('/fireadmin/',
      '').replace(':sub', '')) + (this.props.params && this.props.params.sub
      ? this.props.params.sub
      : '').replace(/\+/g, '/')
    if (key != null) {
    ...
    this.setState({
      pathToDelete: firebasePathToDelete,
        ...
        <div className='card'>
          ...
          <div className='card-content'>
            ...
            <div className='material-datatables'>
              ...
              <Table caller={'firebase'} updateAction={this.updateAction}
                     headers={this.findHeadersBasedOnPath(
  ...
  generateBreadCrumb () {
    ...
    return (<div>{items.map((item, index) => {
      ...
      return (<Link className='navbar-brand' to={path}>{item} <span
        className='breadcrumbSeparator'>{index == items.length - 1
        ? ''
        : '/'}</span>
    ...
    return (
      <div className='content'>
        ...
        <div className='content' sub={this.state.lastSub}>
          ...
          <div className='container-fluid'>
            ...
            <div style={{textAlign: 'center'}}>
              {/* LOADER */}
              {this.state.isLoading ? <Loader color='#8637AD' size='12px'
                                              margin='4px'/> : ''}
            </div>
                ...
                <form className='form-horizontal'>
                  <div className='col-md-12'>
                    <div className='col-md-6'>
                      <button style={{opacity: this.state.isLastPage ? 0.3 : 1}}
                      ...
                      <button style={{opacity: this.state.page > 1 ? 1 : 0.3}}
                              onClick={() => { this.goNext() }} className='btn'>
                        Next
                        <span className='btn-label'>
                    ...
                    <div className='col-md-3'>
                      <a onClick={() => { this.deleteFieldAction(null, false) }}
            ...
            {this.state.fieldsAsArray && this.state.fieldsAsArray.length > 0 ? (
              <div className='col-md-12'>
                <div className='card'>
                  <a onClick={() => { this.refs.simpleDialog.show() }}>
                    <div id='addDiv' className='card-header card-header-icon'
                         data-background-color='purple'
                         style={{float: 'right'}}>
                      <i className='material-icons'>add</i>
                  ...
                  <form className='form-horizontal'>
                    ...
                    {this.state.fieldsAsArray ? this.state.fieldsAsArray.map(
                      (item) => {
                        if (!this.checkIfKeyIsArtifical(item.theKey)) {
                          return (<Fields parentKey={this.getLastPathItem()}
              ...
              <div className='col-md-12'>
                <div className='card'>
                  ...
                  <form method='get' action='/' className='form-horizontal'>
                    <div className='card-header card-header-text'
                         data-background-color='rose'>
                      <h4 className='card-title'>{this.state.lastPathItem +
                      '\' elements'}</h4>
                    </div>
                    <br/>
                    <div className='col-md-12'>
                      {this.state.elements ? this.state.elements.map((item) => {
                        var theLink = '/fireadmin/' + this.state.completePath +
                          Config.adminConfig.urlSeparator + item.uidOfFirebase
                        return (<Link to={theLink}><a
        ...
        <SkyLight hideOnOverlayClicked ref='deleteDialog' title=''>
          ...
          <div className='col-md-12'>
            <Notification type='danger'>All data at this location, incuding
              nested data, will be deleted!</Notification>
          </div>
          <div className='col-md-12'>
            Data Location
          </div>
